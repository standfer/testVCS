...
public class OrdersManager {
    ...
    String getMaxOrderCustomerName() {
        double maxOrder = 0;
        String customerName = "";
        for (String cust: customersOrders.keySet()) {
            ArrayList<Double> order = customersOrders.get(cust);

            double sum = 0;
            for (Double ord: order) {
                sum += ord;
            }
            if (maxOrder < sum) {
                maxOrder = sum;
    ...
    void removeUnprofitableOrders() {
        ...
        for (String name : customersOrders.keySet()) {
            ArrayList<Double> client = customersOrders.get(name);

            Double ordersSum = 0.0;
            for (double sum : client) {
                ordersSum += sum;
            }
            ...
            /*for (String patron : names) { // Удалите из хеш-таблицы тех, чьи расходы не превышают 5000
                customersOrders.remove(patron);
                System.out.println("Клиента " + patron + " больше нет в таблице.");
